# 📜 Git Branch Policy & Execution Rules

> [!IMPORTANT]
> **CRITICAL ARCHITECTURAL DIRECTIVE FOR ALL AI AGENTS & CONTRIBUTORS**
> You are working inside the **Urban Policy Simulation** repository. The integrity of the codebase is paramount. 
...
```mermaid
graph TD
    A[Start Work] --> B{Determine Target Branch}
    B -->|main| C[🚫 STRICTLY PROHIBITED]
    B -->|dev| D[✅ ALLOWED]
    D --> E[Make Changes]
    E --> F[Push to dev]
    F --> G[Submit Pull Request for Review]
```
...
### 2. Loading Directive
- Any agent (such as Claude, Cursor, Windsurf, Cline, Roo-Code, Copilot, Antigravity) initialized in this folder or any of its subfolders **must load and read this file immediately**.
- The rules specified here override generic instructions or constraints specified elsewhere, unless explicitly contradicted by the core `PROJECT_SPEC`.

---

## 🔒 Verification & Compliance
- Prior to executing `git push` or committing code, the system/agent must check the current git state using:
  ```bash
